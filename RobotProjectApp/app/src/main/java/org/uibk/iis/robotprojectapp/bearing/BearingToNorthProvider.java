package org.uibk.iis.robotprojectapp.bearing;
...
/**** Modification from Original version to disable the true north feature( GPS location based bearing) ****/
...
public class BearingToNorthProvider implements SensorEventListener, LocationListener {
	...
	// some arrays holding intermediate values read from the sensors, used to
	// calculate our azimuth
	// value
	...
	private GeomagneticField getGeomagneticField(Location location) {
		GeomagneticField geomagneticField = new GeomagneticField((float) location.getLatitude(), (float) location.getLongitude(),
